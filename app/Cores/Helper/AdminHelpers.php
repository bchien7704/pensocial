...
//        $show_path = route('admin.'.$resource.'.show', ['id' => $id]);
        $edit_path = "";
        if (Route::has('admin.' . $resource . '.edit'))
            $edit_path = route('admin.' . $resource . '.edit', ['id' => $id]);
        $delete_path = "";
        if (Route::has('admin.' . $resource . '.destroy'))
...
if (!function_exists('breadcrumbs')) {
    ...
    function breadcrumbs()
    {
        ...
        if (strpos($route, 'root') === false) {
            ...
            $breadcrumbs .= strpos($route, 'index') !== false ? $parent_text : '<a href="#">' . $parent_text . '</a>';
            $breadcrumbs .= '</li>';
...
if (!function_exists('render_checkbox')) {
    ...
    function render_checkbox($value)
    {
        $ops = "";
        if ($value == 1)
            $ops = '<input type="checkbox" checked="checked" disabled="disabled"/>';
        else
            $ops = '<input type="checkbox" disabled="disabled"/>';
        return $ops;
